source $HOME/.config/nvim/general/sets.vim

source $HOME/.config/nvim/plugins-config/vim-plugins.vim
source $HOME/.config/nvim/plugins-config/vim-coc.vim
source $HOME/.config/nvim/plugins-config/vim-colorizer.vim
source $HOME/.config/nvim/plugins-config/vim-fzf.vim
source $HOME/.config/nvim/plugins-config/vim-gitgutter.vim
source $HOME/.config/nvim/plugins-config/vim-goyo.vim
source $HOME/.config/nvim/plugins-config/vim-highlightedyank.vim
source $HOME/.config/nvim/plugins-config/vim-nerdtree.vim
source $HOME/.config/nvim/plugins-config/vim-taboo.vim

source $HOME/.config/nvim/general/theme.vim
source $HOME/.config/nvim/general/maps.vim

